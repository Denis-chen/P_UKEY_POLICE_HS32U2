...
unsigned short APP_CMD_ReadTerminalInfo(void)
{
	...
	else if(g_APDU.PP2==0x11)
	{
		if(g_APDU.LC!=0x20)
			return SW_LCLE_ERROR;
		cos_get_device_lebal(g_APDU.pOutData);
		g_APDU.RLE=0x20;

	}
	else if(g_APDU.PP2==0xAA)
	{		
...
{	
	...
	memmove(g_APDU.pInData+6,g_APDU.pInData,g_APDU.LC);
	memcpy(g_APDU.pInData,(unsigned char *)&g_APDU.CLA,6);	
	return cos_unblock_userpin(g_APDU.pInData,rand,USER_PIN,g_APDU.PP2);
}
//创建应用的同时创建PIN文件
...
unsigned short APP_CMD_DeleteFinger(void)
{
	...
	{	
		...
		{
			if(xFA_EraseAFinger(g_APDU.pInData[2]-1)==1)	
			{
				ret=SW_SUCCESS;
			}
			else
...
unsigned short APP_CMD_Device_Lebal(void)
{
	g_APDU.RLE=0;
	...
	if(g_APDU.PP1==0)
	{
		return cos_set_device_lebal(g_APDU.pInData);
	}
	else 
	{	
		return SW_P1P2_ERROR;	
...
/*************************************************************************
		...
		功能:	
*************************************************************************/
unsigned short CMD_ReturnBoot(void)
{
	unsigned int ret;
