...
U16 cos_set_device_lebal(U8 *pbin)
{
	U8 *p;
	p=(U8 *)DEV_VER_INFO;
	p+=sizeof(MANUFACTURER_INFO)+sizeof(SERIAL_NUMBER_INFO)+sizeof(KEY_NUMBER_INFO);

	flash_device.writedata((U32)p,pbin,32);
	
	return SW_SUCCESS;
}
U16 cos_get_device_lebal(U8 *pbout)
{
	U8 *p;
	p=(U8 *)DEV_VER_INFO;
	p+=sizeof(MANUFACTURER_INFO)+sizeof(SERIAL_NUMBER_INFO)+sizeof(KEY_NUMBER_INFO);

	flash_device.readdata((U32)p,pbout,32);
...
#if ALG_DEBUG
	UART_printf("SM2   pout\r\n");
	HAL_UART_SEND_ASC((unsigned char*)pout,len+96);
#endif	
	return SW_SUCCESS;
}
