								...
								unsigned char *pbout)
{
	...
	MCSCR |= SSF33_ON;
UART_printf("cbc \r\n");
		HAL_UART_SEND_ASC((U8*)&cbc,1);

	
UART_printf("encrypt \r\n");
		HAL_UART_SEND_ASC((U8*)&encrypt,1);
	UART_printf("pkey \r\n");
		HAL_UART_SEND_ASC((U8*)pkey,16);
	
		UART_printf("pbin \r\n");
		HAL_UART_SEND_ASC((U8*)pbin,len);
	SSF33_CRYPT(pbin, len,(unsigned long *) pkey, piv, encrypt, cbc, pbout);
		UART_printf("pbout \r\n");
		HAL_UART_SEND_ASC((U8*)pbout,len);
	MCSCR &= ~SSF33_ON;
#else
	unsigned int csr;
								...
								unsigned char *pbout)
{
	...
	else
	{
		...
		if(0==encrypt)
		{
			...
			for(i=0;i<len;i+=16)
			{
				memcpy(tmp_data,pbin+i,16);
				for(j=0;j<16;j++) 
				{
					tmp_data[j] = tmp_data[j] ^ tmp_iv[j];
				}
				sms4_run((unsigned long *)tmp_data, (unsigned long *)tmp_iv, 16, encrypt);
		...
		{
			sms4_key_expend((unsigned long *)pkey);
			memcpy(tmp_buff,pbin,len);
			memcpy(tmp_data, piv, 16);
			for(i=0;i<len;i+=16)
			{
			//	UART_printf("pbin \r\n");
			//	HAL_UART_SEND_ASC((U8*)pbin+i,16);		
				sms4_run((unsigned long *)(pbin+i), (unsigned long *)plain, 16, encrypt);
				//UART_printf("plain \r\n");
				//HAL_UART_SEND_ASC((U8*)plain,16);	


				//UART_printf("tmp_data \r\n");
				//HAL_UART_SEND_ASC((U8*)(tmp_data),16);	
				for(j=0;j<16;j++) 
				{
					pbout[i+j] = tmp_data[j] ^ plain[j];
				}
				
				memcpy(tmp_data,tmp_buff+i,16);
			} 
		}
...
void HAL_ALG_SM2LoadPara(unsigned char flag)
{
	if(flag)//load allways
	{
		SM2_Init_flag = 1;
		ECC_LoadPara(&pEcc_ctx, (U8*)SM2_Para, ECCData);
	}
	else//don't load agin
	{
		if (SM2_Init_flag == 0)
		{
			SM2_Init_flag = 1;
...
unsigned char hal_alg_SM2encrypt(U8 *Pubkey,U8 *pIn,unsigned int inlen,U8 *pOut,U8 flag)
{
	...
	if(ret==0)
	{
		...
		else if(flag==1)
		{
			memcpy(mac,(unsigned char *)(G_SM2_OUT_BUF+16),32);
			//c3
			memcpy(pOut+64,mac,32);
			//c2
			memcpy(pOut+64+32,(unsigned char *)(G_SM2_OUT_BUF+24),inlen);
		}
