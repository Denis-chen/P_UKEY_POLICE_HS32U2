...
void Set_CSW (unsigned char CSW_Status, unsigned char Send_Permission)
{
  ...
  HAL_USB_BulkIn(4, ((unsigned char *)& CSW), CSW_DATA_LENGTH);
  Bot_State = BOT_IDLE;
}
...
void Transfer_Data_Request(unsigned char* Data_Pointer, unsigned short Data_Len)
{
	...
	HAL_USB_BulkIn(4, Data_Pointer, Data_Len);
...
//	  Length = BlockNbr * Mass_Block_Size[lun];
	...
	if (TransferState == TXFR_ONGOING )
	{
		...
		{					
			...
			HAL_USB_BulkIn(4,Data_Buffer,Mass_Block_Size[lun]);			
  ...
  if (CBW.dSignature == BOT_CBW_SIGNATURE)
  {
	...
	case SCSI_COMMAND:
	  {	
		...
		memset(g_b_MassComm, 0, COMMUNICATION_BUF_LEN);
		HAL_USB_BulkOut(3, g_b_MassComm, &slength);
	...
	//	memcpy(&g_UMSAPDU, &g_APDU, sizeof(APDU));	
	...
	case SCSI_RESPONSE:
	  {		
		if(g_APDU.RLE)
			memcpy(g_b_MassComm, g_APDU.pOutData, g_APDU.RLE);

		if(g_reboot==1)
			g_reboot=2;
		if(g_reboot==3)
			g_reboot=4;
		g_b_MassComm[g_APDU.RLE] = g_APDU.SW1;
		g_b_MassComm[g_APDU.RLE+1] = g_APDU.SW2;

		Transfer_Data_Request(g_b_MassComm,g_APDU.RLE+2);
